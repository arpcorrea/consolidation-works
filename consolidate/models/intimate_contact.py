...
class IntimateContact():
    ...
    def dic_timestep(self, dic, dic0, v, t):
        ...
        dic[(self.meshes.ny1 - 1):(self.meshes.ny1 + 1), 0:] = dic0[(self.meshes.ny1 - 1):(self.meshes.ny1 + 1), 0:] * (
                    1 + C1 * (
                        self.aux + self.P * 10 ** 6 / v[(self.meshes.ny1 - 1):(self.meshes.ny1 + 1), 0:] * t)) ** (
                                                                           1 / 5)
    ...
    def calculate_average_dic(self):
        self.DicAverage = np.sum(self.meshes.Dic[int(self.meshes.ny / 2 - 1):int(self.meshes.ny / 2 + 1), 1:-1]) / (
                    (self.meshes.nx - 2) * 2)
